		...
		if (user == null) {
			...
			if (user.isPermission()) {
				...
				try {
					...
					if (isUpdate) {
						if (!iDBook.equals("") && !name.equals("") &&  !price.equals("")
								&& !iDCategory.equals("") && !author.equals("")) {
							if (image == null) {
								image = bookBo.getBookByID(Integer.parseInt(iDBook)).getImage();
							}
							SimpleDateFormat f = new SimpleDateFormat("yyyy-MM-dd");
//							Date date = f.parse(dateInput);
							int rsUpdate = bookBo.updateBook(Integer.parseInt(iDBook), name, Integer.parseInt(price),
...
//							}
					...
					if (request.getParameter("delete") != null) {
						int rsDelete = bookBo.delelteBook(Integer.parseInt(request.getParameter("delete")));
						if (rsDelete == 0)
							request.setAttribute("rsDelete", 0);
					}

					if (request.getParameter("select") != null) {
						request.setAttribute("bookSelect",
								bookBo.getBookByID(Integer.parseInt(request.getParameter("select"))));
					}
