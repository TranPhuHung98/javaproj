...
<body>
 ...
 %> <%=user.getUserName()%>
					...
					<li><a href="AuthController?logout=true">Đăng Xuất</a></li>
					<%
						} else {
					%>
					Đăng Nhập
					...
					%>
					<li><a href="CartController"
						style="color: white; font-weight: bold; text-decoration: none;">
							Giỏ Hàng (<%=sumProduct%>)
					</a></li>

					<li><a href="HistoryOrderController"> Lịch sử mua hàng </a></li>
				</ul>
			</div>
			<!-- /.navbar-collapse -->
		</div>
		...
		<div class="row">
			...
			<div class="col-md-3">
				...
				<div class="list-group">
					...
					%>
					<a
						href="CategoryController?idcategory=<%=category.getIDCategory()%>"
						class="list-group-item"> <%=category.getName()%>
					</a>
					<%
						}
						}
					...
					<table class="table" align="center" border="1">
						<tbody>
							...
							<tr style="text-align: center; font-weight: bold">
								...
								<form
									action="CartController?action=update&idcart=<%=cart.getIdCart()%>"
									method="post">
									...
									<td><%=Base.getPriceFormat(cart.getQuantity() * cart.getBook().getPrice())%>
									</td>
									<td><a
										href="ProductDetailController?idbook=<%=cart.getBook().getIDBook()%>">
											Chi tiết </a></td>
									<td><a
										href="CartController?action=delete&idcart=<%=cart.getIdCart()%>">Xóa</a></td>
							...
							<tr style="font-weight: bold; text-align: right; color: red">
								<td colspan="4">Số lượng sách: <%=sumProduct%>
								</td>
								<td colspan="5">Tổng tiền: <%=Base.getPriceFormat(sumPrice)%>
								</td>
							</tr>
							<tr style="font-weight: bold; color: blue; text-align: right">
								<td colspan="9"><a href="#">Xóa Giỏ Hàng</a></td>
							</tr>
							<tr style="font-weight: bold; color: blue; text-align: right">
								<td colspan="9" align="center"><a
									href="CartController?action=order">ĐẶT HÀNG</a></td>
							</tr>
						</tbody>
					</table>
				</div>
